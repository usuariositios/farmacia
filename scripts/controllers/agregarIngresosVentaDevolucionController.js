...
{
app.controller('agregarIngresosVentaDevolucionController',
    ['$scope','Data','$location','$timeout','DataCont','$window', 
        function ($scope, Data, $location,$timeout,DataCont,$window) {
    //guardar el ingreso almacen por devolucion
    //devolver las cantidades a las etiquetas que corresponde
    ...
    $scope.salidasVenta =JSON.parse(sessionStorage.getItem("salidasVenta"));//de esta salida se tiene que devolver
    console.log($scope.salidasVenta);
    $scope.salidasVentaTotal = angular.copy($scope.salidasVenta);//para los totales
    $scope.productos = {};
    ...
    //console.log($scope.salidasVenta);
    Data.get('/devolucion/devolucion').then(function(devolucion){
        
        $scope.devolucion = angular.copy(devolucion);
        //$scope.devolucion.codDevolucion = 0;//llamar a post
        $scope.devolucion.nroDevolucion = 0;//llamar a post
        $scope.devolucion.almacenesVenta = angular.copy($scope.salidasVenta.almacenesVenta);
        $scope.devolucion.clientes = angular.copy($scope.salidasVenta.clientes);
        $scope.devolucion.facturasEmitidas.codFacturaEmitida = $scope.salidasVenta.codFacturaEmitida;
        $scope.devolucion.facturasEmitidas.nroFactura = $scope.salidasVenta.nroFactura;
        $scope.devolucion.fechaDevolucion =fechaActualDDMMYYYY();
        $scope.devolucion.gestion = angular.copy($scope.salidasVenta.gestion);
        $scope.devolucion.estadosDevolucion.codEstadoDevolucion = 1;
        Data.post('/devolucion/nroDevolucion', $scope.devolucion).then(function (data) {            
            $scope.devolucion.nroDevolucion = data;
            console.log($scope.devolucion);            
        });
        
    });
    
    Data.get('/devolucionDetalle/devolucionDetalle').then(function(devolucionDetalle){
        $scope.devolucionDetalleObj = angular.copy(devolucionDetalle);
        console.log($scope.devolucionDetalleObj);
    });
    //datos de ingreso venta
    dataGet("/ingresosVenta/ingresosVenta",function(data) {
            console.log(data);
            $scope.ingresosVenta = data;
            $scope.ingresosVenta.gestion = angular.copy($scope.salidasVenta.gestion);
            $scope.ingresosVenta.almacenesVenta = angular.copy($scope.salidasVenta.almacenesVenta);
            $scope.ingresosVenta.estadosRegistro.codEstado= 1;
            $scope.ingresosVenta.estadosIngresoVenta.codEstadoIngresoVenta= 1;
            $scope.ingresosVenta.tiposIngresosVenta.codTipoIngresoVenta = 3;//por devolucion
            
            dataPost("/ingresosVenta/nroIngresoVenta",$scope.ingresosVenta, function(data) {
    ...
    Data.get('/salidasVentaDetalle/salidasVentaDetalle').then(function(data){
        ...
        Data.post('/salidasVentaDetalle/cargarSalidasVentaDetalle',sad).then(function(data){
            $scope.salidasVentaDetalleList = data;
    ...
    $scope.guardarDetalleCantSacar_action = function(){
        ...
        for(i=0;i<$scope.sadSeleccionado.salidasVentaDetalleIngresosList.length;i++){            
            if(isNaN(parseFloat($scope.sadSeleccionado.salidasVentaDetalleIngresosList[i].cantRetornado))){
                $.growl.warning({title:"ADVERTENCIA!", message: "Registre cantidades validas" });
                return null;
            }
            totalSd = parseFloat(totalSd) + parseFloat($scope.sadSeleccionado.salidasVentaDetalleIngresosList[i].cantRetornado);            
        }
        if(totalSd<=0){
        ...
        //   calculaDescuentoDetalle($scope.sadSeleccionado);            
        //    obtieneCantidadTotal();
        $scope.sadSeleccionado.cantRetornar = totalSd;
        
        $('#detalleEtiquetasDialog').modal('hide');
        return null;
    };
    $scope.cantEtiquetas_change = function(e){
        if(isNaN(parseFloat(e.cantRetornado))){
                $.growl.warning({title:"ADVERTENCIA!", message: "Registre cantidades validas" });                
                return null;
        }
        if(parseFloat(e.cantRetornado)>parseFloat(e.cantSacar)){
            $.growl.warning({title:"ADVERTENCIA!", message: "la cantidad excede lo vendido" });
            e.cantRetornado = parseFloat(e.cantSacar);
            return null;
        }
        console.log(e);
    ...
    $scope.guardarDevolucionVenta_action = function(){            
            ...
            for (var j = 0; j < $scope.salidasVentaDetalleList.length; j++) {
                ...
                for (var i = 0; i < sd.salidasVentaDetalleIngresosList.length; i++) {
                    var sadi = sd.salidasVentaDetalleIngresosList[i];
                    sadi.ingresosVentaDetalle.cantRestante = parseFloat(sadi.ingresosVentaDetalle.cantRestante) + parseFloat(sadi.cantRetornado);//retornado hacia almacen
                }
            }
            ...
            Data.post("/salidasVenta/editarSalidasVentaBusiness", $scope.salidasVentaBusiness).then(function(data){
                //window.location = "navegadorSalidasVenta.html";
                $location.path("navegadorIngresosVentaDevolucion");
            });
            $scope.devolucionBusiness = {};
            $scope.devolucionBusiness.devolucionesDetalleList = [];
            //guardar el ingreso y el retorno           
            $scope.devolucionBusiness.devoluciones = angular.copy($scope.devolucion);
            $scope.ingresoVentaBusiness = {};
            $scope.ingresoVentaBusiness.ingresosVenta = angular.copy($scope.ingresosVenta);
            $scope.ingresoVentaBusiness.ingresosVentaDetalleList = [];
            
            //generar detalle de devolucion
            for (var j = 0; j < $scope.salidasVentaDetalleList.length; j++) {//el detalle de salida editada
                var sd = $scope.salidasVentaDetalleList[j];
                var dt = angular.copy($scope.devolucionDetalleObj);
                dt.devoluciones = angular.copy($scope.devolucion);
                dt.codDevolucionDetalle = j+1;//detalle
                ...
                //datos de detalle ingreso venta
                var id = angular.copy($scope.ingresosVentaDetalle);
                id.productos = angular.copy(sd.productos);
                id.almacenesVenta = angular.copy($scope.salidasVenta.almacenesVenta);
                id.cantIngreso = parseFloat(sd.cantidadRetornar);
                id.totalMonto = parseFloat(sd.montoTotal);
                id.unidadesMedida = angular.copy(sd.unidadesMedida);
                id.costoUnitario = parseFloat(sd.costoUnitario);
                $scope.ingresoVentaBusiness.ingresosVentaDetalleList.push(id) ;
            }
            
            
            
            Data.post('/devolucion/guardarDevolucionBusiness',$scope.devolucionBusiness).then(function(devolucion){
              console.log(devolucion);
            });
            
            Data.post("/ingresosVenta/guardarIngresosVentaBusiness", $scope.ingresoVentaBusiness).then(function(data){
                    console.log(data);
                    $location.path("navegadorIngresosVenta");                    
                });
            
            //generar datos de ingreso venta por devolucion
