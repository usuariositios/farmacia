...
{
app.controller('reporteLibroVentasController',
...
function ($scope, Data, $location,$window) {
    ...
    $scope.verReporte = function(){
        ...
        $window.open("/FARMACIA_REPORTES/libroVentas/reporteLibroVentas.jsp?codGestion="+$scope.datosReporte.codGestion +"&nombreGestion="+$scope.datosReporte.nombreGestion.trim()+ "&codMes="+$scope.datosReporte.codMes +"&nombreMes="+$scope.datosReporte.nombreMes.trim()+"&codEmpresa="+$scope.usuarioPersonal.empresas.codEmpresa);
    };
